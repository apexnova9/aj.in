...
export function TechExpertise() {
  return (
    <div>
      <h2 className="text-2xl font-bold mb-8 dark:text-white">Technical Expertise</h2>
      <div className="grid md:grid-cols-2 gap-6">
        {expertiseAreas.map((area) => (
          <div key={area.title} className="p-6 rounded-xl bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
            ...
            <h3 className="text-xl font-semibold mb-2 dark:text-white">{area.title}</h3>
            <ul className="space-y-2 mb-4">
              {area.skills.map((skill) => (
                <li key={skill} className="text-slate-600 dark:text-slate-300">{skill}</li>
              ))}
            </ul>
            <p className="text-slate-500 dark:text-slate-400">{area.description}</p>
          </div>
        ))}
      </div>
    </div>
