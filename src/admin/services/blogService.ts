...
export const blogService = {
  ...
  async getPost(id: number): Promise<BlogPost> {
    try {
      const response = await fetch(API_ENDPOINTS.POSTS.GET(id));
      if (!response.ok) {
  ...
  async getPostBySlug(slug: string): Promise<BlogPost> {
    try {
      ...
      const response = await fetch(`${API_URL}${API_ENDPOINTS.POSTS.LIST}/${slug}`);
  ...
  async createPost(data: BlogPostInput): Promise<BlogPost> {
    try {
      ...
      const response = await fetch(`${API_URL}${API_ENDPOINTS.POSTS.CREATE}`, {
        method: 'POST',
        body: formData,
      });
  ...
  async updatePost(id: number, data: BlogPostInput): Promise<BlogPost> {
    try {
      ...
      const response = await fetch(`${API_URL}${API_ENDPOINTS.POSTS.UPDATE(id)}`, {
        method: 'PUT',
        body: formData,
      });
  ...
  async deletePost(id: number): Promise<void> {
    try {
      const response = await fetch(`${API_URL}${API_ENDPOINTS.POSTS.DELETE(id)}`, {
        method: 'DELETE',
      });
